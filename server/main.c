...
static void sighandler(int signal) { exit(0); }
...
int main(int argc, char **argv) {
  ...
  if (WSAStartup(MAKEWORD(2, 2), &_d)) {
    fputs(
        "Fatal error while initializing server: Failed to initialize WinSock2",
        stderr);
    return -1;
  }
#else
      ...
      for (unsigned int j = 1; argv[argi][j] != '\0'; j++) {
        ...
        case 'h':
          printf(
              ...
              "\n"
              "Options:\n"
              "-c <certfile>          Specifies a certificate file to use for "
              "TLS "
              "                       encryption. Requires -k to be specified "
              "as well. \n"
              "                       Note that TLS is a work in progress, and "
              "is \n"
              "                       currently only supported by the Python "
              "client.\n"
              "-d                     Forks the process to the background Only "
              "works on \n"
              "                       Unix-like systems.\n"
              "-h                     Displays this help message.\n"
              "-k <privkeyfile>       Specifies a private key file to use for "
              "TLS \n"
              "                       encryption. Requires -c to be specified "
              "as well. \n"
              "                       Note that TLS is a work in progress, and "
              "is \n"
              "                       currently only supported by the Python "
              "client.\n"
              "-l <protocol>:<info>   Specifies an additional method for "
              "clients to \n"
              "                       connect to the server. Valid protocols "
              "are unix, \n"
              "                       ipc (an alias for unix), tcp, and ws. "
              "Additional \n"
              "                       information is either a socket file (in "
              "the case \n"
              "                       of unix) or a port on the local machine "
              "(in the \n"
              "                       case of tcp and ws). The unix protocol "
              "only works \n"
